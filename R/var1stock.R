...
function(){
  ...
  my.draw <- function(panel) {
   ...
   if (panel$distribution=="normal"){
     variable= (qnorm(conf)*sigma*sqrt(horizon)-mu)*price
   }
    else{
     variable= (1-(exp(mu-qnorm(conf)*sigma*sqrt(horizon)))) *price
    }
    var<-round(var,2)
    plot(1:20, 1:20, type="n", xlab="", ylab="",
         axes=FALSE, frame = TRUE)
    text(10, 10, paste("variable= ", var, sep=""),cex=1.5)
