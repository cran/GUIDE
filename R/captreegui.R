...
function(){
    ...
    my.draw <- function(panel){
      ...
      captree <- function(FV,strike,optmaturity,coupon,r0,u,d,q){
        ...
        for (col in seq(from = ncols-nsteps-margin-thisstep, to=ncols-nsteps-margin+thisstep, by = 2)) {
          ...
          O[lastrow, col] = max(Rates[lastrow, col]- strike,0)/(1+Rates[lastrow, col]/100)
        ...
        capvalue= O[startrow,ceiling(ncols/2)]*FV
        capvalue=round(capvalue,2)
      ...
      C = captree(FV,strike,optmaturity,coupon,r0,u,d,q)[[2]]
      val = captree(FV,strike,optmaturity,coupon,r0,u,d,q)[[1]]
      ...
      title(main = paste(floor(nrows/2),"Step ", topaste, " Tree. Cap value = ", val))
      panel
    }   
