...
function(){
  ...
  bonddur<- function(panel,couprate,faceval){
  ...
  my.draw <- function(panel) 
  {
    with(panel, {
      bonddurs<-rep(0,20)
      couprates<-seq(1,20,1)
      for (i in 1:20){
        bonddurs[i]=bonddur(panel,couprates[i],faceval)
      }
      
      plot(couprates, bonddurs, type='l',lwd=2, xlab="Coupon Rate", ylab="Duration",
           frame = TRUE)
      title(paste("Duration and Coupon Rate"))
      #cat(pv)
    })
    panel
  ...
  my.panel <- rp.control("Duration and Coupon Rate")
  ...
  rp.doublebutton(my.panel,variable=discrate,step=0.5,title="Discount Rate (%  p.a.)",initval=10,range=c(1,15),showvalue=TRUE,action=my.redraw)
